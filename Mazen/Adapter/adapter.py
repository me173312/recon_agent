...
class BackendAdapter:
    ...
    def _validate_messages(self, messages: list[dict]) -> None:
        ...
        for index, message in enumerate(messages):
            ...
            if "content" not in message and "tool_calls" not in message:
                raise ValueError(
                    f"Message at index {index} must contain 'content' or 'tool_calls'."
                )

            if role == "tool" and "tool_call_id" not in message:
                raise ValueError(
                    f"Message at index {index} with role 'tool' is missing 'tool_call_id'."
                )
    ...
    def _send_request(self, messages: list[dict], tools: list | None = None) -> Any:
        ...
        try:
            ...
            logger.exception("Rate limit or quota exceeded.")
            raise RuntimeError("Quota exceeded or rate limit reached.") from e
            ...
            raise RuntimeError("Invalid API Key or unauthorized access.") from e

        except Exception as e:
            logger.exception("Unexpected error occurred while communicating with LLM")
